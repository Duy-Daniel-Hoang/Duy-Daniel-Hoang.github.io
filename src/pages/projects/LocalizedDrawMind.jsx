...
function LocalizedContent({ c }) {
  ...
  return <>
    ...
    <main>
      ...
      <section className="case-section"><div className="wrap"><h2 className="case-h2"><span className="case-num">02</span>{c.impactTitle}</h2><div className="impact-list">{c.impacts.map((impact, index) => <div className="impact-entry" key={impact}><div className="impact-item"><div className="impact-mark">▸</div><div className="impact-text">{impact}</div></div>{index === 1 && <MapComparisonChart />}</div>)}</div></div></section>
      <section className="case-section"><div className="wrap"><h2 className="case-h2"><span className="case-num">03</span>{c.domainTitle}</h2><div className="case-body">{c.domain.map((p) => <p key={p}>{p}</p>)}</div><div className="compare-grid">
      ...
      <section className="case-section"><div className="wrap"><h2 className="case-h2"><span className="case-num">04</span>{c.approachTitle}</h2><div className="case-body"><p>{c.approach}</p></div><AgentFlow steps={steps} /><div className="stack-row">{["Object Detection", "RT-DETR", "ResNet-101", "Domain Adaptation", "CVAT", "Error Analysis", "Post-processing"].map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div></div></section>
      <section className="case-section"><div className="wrap"><h2 className="case-h2"><span className="case-num">05</span>{c.systemTitle}</h2><div className="case-body"><p>{c.detect}</p></div><div className="case-video-frame" style={{ marginTop: 20 }}><video ref={detectionRef} className="case-video" src="/assets/drawmind/det-seg-vid.mp4" poster="/assets/drawmind/det-seg-poster.webp" loop muted playsInline preload="metadata" /></div><div className="diagram-caption">{c.detectCaption}</div>
