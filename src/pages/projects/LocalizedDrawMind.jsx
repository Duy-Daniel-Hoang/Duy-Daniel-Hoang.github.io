...
function LocalizedContent({ c }) {
  ...
  return <>
    ...
    <main>
      ...
      <section className="case-section"><div className="wrap"><h2 className="case-h2"><span className="case-num">04</span>{c.domainTitle}</h2><div className="case-body">{c.domain.map((p) => <p key={p}>{p}</p>)}</div><div className="compare-grid">
      ...
      <section className="case-section"><div className="wrap"><h2 className="case-h2"><span className="case-num">05</span>{c.approachTitle}</h2><div className="case-body"><p>{c.approach}</p></div><AgentFlow steps={steps} /><div className="stack-row">{["Object Detection", "RT-DETR", "ResNet-101", "Domain Adaptation", "CVAT", "Error Analysis", "Post-processing"].map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div></div></section>
    </main>
